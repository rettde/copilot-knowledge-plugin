...
2. **Execute independent tasks** — Work through tasks that don't depend on each other. For single tasks or tasks requiring heavy interaction, execute inline.
...
- **Use what's available.** If there are tools, APIs, or integrations that can help, use them. Don't recreate what exists.
- **Follow project conventions.** Check `.github/copilot-instructions.md` for style guides, data sources, tool preferences.
...
**Critical:** Write the execution log to the plan file after each batch. Do not keep it only in conversation. This log feeds `#kw-compound` with concrete material to extract learnings from.
