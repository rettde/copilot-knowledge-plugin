...
### Step 2: Research (run all searches simultaneously)

**2a. Past work** — Follow the instructions in `#research-past-work`:
- Search `plans/` and `docs/solutions/` for related past work
- Pass: the plan request description + work type from Step 1

**2b. Knowledge base** — Follow the instructions in `#research-knowledge-base`:
- Search `docs/knowledge/` for saved learnings
- Pass: the plan request description + keywords

**2c. External research** (if the topic benefits from outside context):
...
**2d. Live data** — Pull current metrics if the plan involves data. Follow whatever data hierarchy exists in the project's `.github/copilot-instructions.md`. If none exists, ask the user where to find relevant data.
...
> "Plan written to `plans/{filename}`. What next?"
>
...
- **Don't over-template.** The template is a starting point. Skip sections that don't apply.
