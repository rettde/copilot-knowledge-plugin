...
Ask **1-3 load-bearing questions at once** (never more than 3). Frame each question with options drawn from what surfaced in the brainstorm — don't ask open-ended questions when you already have candidate answers.
...
**Critical:** If "Save" or "Run `#kw-plan`" is selected, ALWAYS write the brainstorm to `plans/brainstorm-{descriptive-name}.md` first. This file becomes the origin document that `#kw-plan` will search for. Never skip the file write.
