...
- Search `docs/knowledge/` for key phrases
- Search `docs/solutions/` for key phrases
...
After identifying what to save, follow the instructions in `#research-stale-knowledge`:
...
**Critical:** Only the compound workflow writes files — both new learnings and updates to stale entries.
...
- **Tags are for retrieval.** Choose tags that `#kw-plan`'s search would match on. Think: "What future question would this answer?"
